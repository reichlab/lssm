## functions for estimating linear state space models

#' Estimate a linear state space model
#'
...
#' @param model string specifying model to fit: one of 'arma' or
#'   'spline_smoother'
...
fit_lssm <- function(
  ...
  if(!is.ts(y) && missing(ts_frequency)) {
    stop(paste0("If y is not an object of class 'ts', the ts_frequency ",
  ...
  model <- match.arg(model, c("arma", "spline_smoother"))
  ...
  if (identical(transformation, "box-cox")) {
    est_bc_params <- car::powerTransform(
      y + transform_offset,
      family = "bcPower")
  ...
  for (param_name in params_to_save) {
    attr(lssm_fit, paste0("lssm_", param_name)) <- get(param_name)
  }
  if (identical(transformation, "box-cox")) {
    attr(lssm_fit, "lssm_bc_lambda") <- est_bc_lambda
  }

  class(lssm_fit) <- c("lssm", class(lssm_fit))
